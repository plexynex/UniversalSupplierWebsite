        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            try {
                const savedDark = localStorage.getItem('usl_dark_mode');
            ...
            if (darkToggle) {
                darkToggle.addEventListener('click', function() {
                    ...
                    try { localStorage.setItem('usl_dark_mode', isDark ? '1' : '0'); } catch (e) {}
                    applyDarkMode(isDark);
                });
            }

            // Highlight primary CTA with interactive animation
            if (primaryCTA) {
                // add highlight for a short burst on load
                primaryCTA.classList.add('highlight');
                setTimeout(() => primaryCTA.classList.remove('highlight'), 4000);

                // subtle scale on hover and reapply pulse after hover out
                primaryCTA.addEventListener('mouseenter', () => {
                    primaryCTA.classList.remove('highlight');
                    primaryCTA.style.transform = 'translateY(-3px) scale(1.02)';
                });
                primaryCTA.addEventListener('mouseleave', () => {
                    primaryCTA.style.transform = '';
                    // reapply a single pulse later
                    setTimeout(() => primaryCTA.classList.add('highlight'), 800);
                    setTimeout(() => primaryCTA.classList.remove('highlight'), 2800);
                });
            }
